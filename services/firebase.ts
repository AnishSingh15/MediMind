...
export async function signInWithEmail(email: string, password: string): Promise<User> {
    ...
    await AsyncStorage.setItem('@medimind_userId', user.uid);
    await AsyncStorage.setItem('@medimind_userName', user.displayName || email.split('@')[0]);
    await AsyncStorage.setItem('@medimind_userPhoto', user.photoURL || '');
...
export async function addMedicine(userId: string, medicineData: MedicineData): Promise<string> {
    const firestore = getDb();
...
export async function updateMedicine(
    ...
    await updateDoc(doc(firestore, 'users', userId, 'medicines', medicineId), updateData);
}
...
export async function getMedicines(userId: string): Promise<(MedicineData & { id: string })[]> {
    const firestore = getDb();
    const snapshot = await getDocs(collection(firestore, 'users', userId, 'medicines'));
...
export async function logDose(userId: string, logData: DoseLogData): Promise<string> {
    const firestore = getDb();
    const ref = await addDoc(
...
export async function getTodayLogs(userId: string, date: string): Promise<(DoseLogData & { id: string })[]> {
    const firestore = getDb();
    const q = query(
        collection(firestore, 'users', userId, 'logs'),
