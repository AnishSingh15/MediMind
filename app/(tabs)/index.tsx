...
import { rescheduleAllIfMissing, showBatteryOptimizationGuide } from '../../services/notifications';
...
export default function HomeScreen() {
  ...
  const todaySchedule = useMemo(() => {
    ...
    for (const med of activeMedicines) {
      ...
      if (med.frequency === 'weekdays' && (dayOfWeek === 0 || dayOfWeek === 6)) {
        continue;
      }
      if (med.frequency === 'custom' && !med.customDays.includes(dayOfWeek)) {
        continue;
      }

      for (const time of med.times) {
        const log = todayLogs.find(
          (l) => l.medicineId === med.id && l.scheduledTime === time && l.scheduledDate === today
        );
        items.push({
          medicineId: med.id,
  ...
  const upcoming = todaySchedule.filter((item) => !item.isTaken && !item.isSkipped);
  const completed = todaySchedule.filter((item) => item.isTaken || item.isSkipped);
